...
from waterbutler.providers.intgromat import settings
from waterbutler.providers.intgromat.metadata import intgromatRevision
from waterbutler.providers.intgromat.metadata import intgromatFileMetadata
from waterbutler.providers.intgromat.metadata import intgromatFolderMetadata
from waterbutler.providers.intgromat.metadata import intgromatFolderKeyMetadata
from waterbutler.providers.intgromat.metadata import intgromatFileMetadataHeaders


class intgromatConnection(S3Connection):
    def __init__(self, aws_access_key_id=None, aws_secret_access_key=None,
        ...
        super(intgromatConnection, self).__init__(aws_access_key_id,
...
class intgromatProvider(provider.BaseProvider):
    """Provider for Integromat service.
    ...
    * A GET prefix query against a non-existent path returns 200
    ...
    def __init__(self, auth, credentials, settings):
        """
        .. note::
            Neither `intgromatConnection#__init__` nor `intgromatConnection#get_bucket`
            sends a request.
        ...
        if m is not None:
            host = m.group(1)
            port = int(m.group(2))
        self.connection = intgromatConnection(credentials['access_key'],
    ...
    async def intra_copy(self, dest_provider, source_path, dest_path):
        """Copy key from one Integromat bucket to another. The credentials specified in
    ...
    async def upload(self, stream, path, conflict='replace', **kwargs):
        """Uploads the given stream to Integromat
        :param waterbutler.core.streams.RequestWrapper stream: The stream to put to Integromat
    ...
    async def create_folder(self, path, folder_precheck=True, **kwargs):
        ...
        async with self.request(
            ...
            return intgromatFolderMetadata({'Prefix': path.path})
    ...
    async def _metadata_file(self, path, revision=None):
        ...
        await resp.release()
        return intgromatFileMetadataHeaders(path.path, resp.headers)
    ...
    async def _metadata_folder(self, path):
        ...
        for content in contents:
            ...
            if content['Key'].endswith('/'):
                items.append(intgromatFolderKeyMetadata(content))
            else:
                items.append(intgromatFileMetadata(content))
